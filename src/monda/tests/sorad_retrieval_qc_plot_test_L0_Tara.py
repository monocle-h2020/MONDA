...
def run_example(platform_id = 'PML_SR002',
              ...
              #  start_time = datetime.datetime(2023,9,20,23,59,59),
                end_time = datetime.datetime(2023,11,11,23,59,59),
                bbox = None,
                target='So-Rad_L0', rrsalgorithm='3c',
...
def run_example_L0(platform_id = 'PML_SR002' ,
                   start_time = datetime.datetime(2023,4,1,00,0),
                   end_time = datetime.datetime(2023,11,11,23,59,59),
                   bbox = None,
                   target='So-Rad_L0', rrsalgorithm='3c',
                   output_radiance = True,
    ...
    """
    Download L0 data from a specific So-Rad platform 
    ...
    layer = 'rsg:sorad_public_l0'  # hardcoded L0 layer
    ...
    for i in range(days):
        ...
        ed_sensor_id, lt_sensor_id, ls_sensor_id, ed_l0, ls_l0, lt_l0  = unpack_response_L0(response)
 
        ed_filename = os.path.join(target, file_id + '_'  + str(ed_sensor_id) + '_Ed_L0.csv')
        lt_filename = os.path.join(target, file_id + '_'  + str(lt_sensor_id) + '_Lt_L0.csv')
        ls_filename = os.path.join(target, file_id + '_'  + str(ls_sensor_id) + '_Ls_L0.csv')
        ...
        if os.path.exists(lt_filename):
         ...
         #if os.path.exists(ed_filename):
          #   log.warning(f"File {ed_filename} was overwritten")
        np.savetxt(ed_filename, ed_l0, delimiter=',', fmt='%.8f')
        np.savetxt(lt_filename, lt_l0, delimiter=',', fmt='%.8f')
        np.savetxt(ls_filename, ls_l0, delimiter=',', fmt='%.8f')
...
def unpack_response_L0(response):
    """
    Unpack the L0 WFS response
    """
    log.info(response['result'][0].keys())   # uncomment to show all available fields
   # breakpoint()

  # meta data given in L1
  ...
  #  tilt_std      = np.array([response['result'][i]['tilt_std'] for i in range(len(response['result']))])
    ...
    ed_sensor_id  = np.array(response['result'][0]['l0_ed_sensor_id'])
    lt_sensor_id  = np.array(response['result'][0]['l0_lt_sensor_id'])
    ls_sensor_id  = np.array(response['result'][0]['l0_ls_sensor_id'])

    ed_l0 = access.get_l0spectra(response, 'ed') # irradiance spectra in 2D matrix format: rows time index, columns wavelength
    ls_l0 = access.get_l0spectra(response, 'ls')
    lt_l0 = access.get_l0spectra(response, 'lt')

    return ed_sensor_id, lt_sensor_id, ls_sensor_id, ed_l0, ls_l0, lt_l0 
