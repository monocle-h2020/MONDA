...
Data access is provided from sorad_access.py, quality control is provided from sorad_qc.py
and plots is provided from sorad_plots.py
...
Both algorithm products have separate quality control chains and plot functions: use the -a argument to specify which should be shown.

Example usage via command line (see parse-args function in sorad_access
    test_wfs_fpand3c.py  -p PML_SR004 -a 3c -i 2021-08-16 -f 2021-08-22 -c True -t test_folder 

This will produce results from So-Rad platform with serial number PML_SR004 using 3C Rrs processing scheme between 2021-08-16 
and 2021-08-22 saved to test folder
...
def run_example_fp(platform_id = 'PML_SR004', final_day = datetime.date.today() - datetime.timedelta(days=1), initial_day = datetime.date.today() - datetime.timedelta(days=1), target='.', spectra = False):
    ...
    for i in range(days):
        ...
        if response['length'] > 0:
            ...
            wl = np.arange(response['result'][0]['wl_grid'][0], response['result'][0]['wl_grid'][1]-1, response['result'][0]['wl_grid'][2])  # core metadata
            ...
            # q_keys = [i for i in locals() if i.startswith('q_')] # optional conde that can output all qc masks
            # for i in range(len(q_keys)): # add all qc fields to the dataframe
            # d[str(q_keys[i])] = pd.Series(eval(q_keys[i]))
          
            if target != '.': # save output (on daily basis)
...
def run_example_3c(platform_id = 'PML_SR004', final_day = datetime.date.today() - datetime.timedelta(days=1), initial_day = datetime.date.today() - datetime.timedelta(days=1), target = '.', spectra = False):
    ...
    for i in range(days):
        ...
        if response['length'] > 0:
            ...
            q_rho = qc.qc_3c_rho_filter(rho_ds,rho_dd,rho_s,upperbound = 0.1) # removes
