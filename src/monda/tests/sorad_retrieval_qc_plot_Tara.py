...
def run_example(platform_id = 'PML_SR002',
                start_time = datetime.datetime(2023,4,1,0,0,0),
                end_time = datetime.datetime(2023,9,1,23,59,59),
                bbox = None,
                target='Tara_output_newcals', rrsalgorithm='3c',
...
def unpack_response(response, rrsalgorithm, wl_out):
    ...
    print(log.info(response['result'][0].keys())) 
    ...
    if rrsalgorithm == '3c':
       ...
       # print(np.median(np.array([response['result'][i]['c3_rho_s'] for i in range(len(response['result']))])))
       ## print(np.median(np.array([response['result'][i]['c3_rho_ds'] for i in range(len(response['result']))])))
       # print(np.median(np.array([response['result'][i]['c3_rho_dd'] for i in range(len(response['result']))])))
